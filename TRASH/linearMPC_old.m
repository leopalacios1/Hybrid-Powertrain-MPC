function [powerFC,compTime] =  slow_linearMPC(SoC,C_rate,V_bat,P_load,V_fc, time)

        global Ts SoC_Max SoC_Min C_rate_Max C_rate_Min uMax uMin StatesMin StatesMax  InputsMin InputsMax Hp Q R controller;
if time > 0.000000001  
...
%               x = A*x + B*u{j} + Bd*P_load;
%             end
%             objective =objective + ((r-x)./normalX)'*Q*((r-x)./normalX) + u{j}./normU'*R*u{j}./normU;% objective + (r-x(1:2))'*Q*(r-x(1:2)) + u{j}'*R*u{j};
% 
